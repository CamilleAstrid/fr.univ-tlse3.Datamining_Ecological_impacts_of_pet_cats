...
#_________________Chargement START_________________#

#--------------------------------------------------------------------------------#
# Source : https://r-coder.com/progress-bar-r/                                   #
#                                                                                #
# Ce code permet de visualiser une barre de progression et donc de s'assurer que #
# l'algorithme tourne toujours dans le cas d'un volume de données conséquent.    #
#--------------------------------------------------------------------------------#

listeIndexNA = list() ; n_iter = length(tb[,1])
...
for(i in 1:n_iter) {
  ...
  # Code to be executed
  #---
  for (element in c("N.pray","Hrs.indors","N.neigbours")){
    if (is.na(tb[i, element])){
...
#_________________Suppression ligne contenant les NA_________________#
...
fileConn <- file('export_tb.acp_summary.txt')
...
#                             TO DO                                   #
#######################################################################
#_________________Visualisation..._________________#
g <- tb %>% 
  ggplot(aes(Comp1, Comp2, color=Hunt, shape=Hunt)) +
  geom_point() +
  theme_light() +
  ggtitle('ACP normée chats australiens')
ggsave(filename = "ACP_normee_CatsAust.png", scale = 3, plot = g)

#_________________...avec factoextra_________________#
g <- tb.acp %>%
  fviz_pca_ind(label = "none", # hide individual labels
               habillage = tb$cultivar, # color by groups
               palette = c("#17202a", "#2ecc71", "#e74c3c"),
               addEllipses = TRUE # Concentration ellipses
  )
ggsave(filename = "ACP_normee_CatsAust_factoextra", scale = 3, plot = g)
...
as.matrix(mdist)[1:10, 1:10] %>% round(2)
