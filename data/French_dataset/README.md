# Jeux de données

Ce dossier contient les fichiers de données de test, afin d'éprouver le modèle.
Ce jeu de données a été récolté à l'aide des informations récoltées par les propriétaires de chats vivant en France.

## Sources
Nous avons demandé à des propriétaires de chats de nous remplir une fiche afin de collecter toutes les informations nécessaires à l'analyse.
De même, leur accord à été formulé par écrit et signé par les propriétaires et l'équipe de recherche.

## Description
Il s'agit de chats vivant dans le sud ouest de la France.
De nombreuses caractéristiques des chats étudiés y sont rencensées telles que l'âge, le sexe, les habitudes de chasse, etc.

## Récolte

Mail envoyé aux participants :
```
Propriétaire(s) de chat(s) bonjour,

Dans le cadre de notre projet de recherche intitulé Félinomicon – L’algorithme occulte du félin moderne, mené au sein du Master 1 Bioinformatique et Biologie des Systèmes de l’Université Toulouse III – Paul Sabatier (année universitaire 2024-2025), nous réalisons une étude sur l’impact écologique des chats domestiques sur la biodiversité locale.

Cette étude repose notamment sur la collecte de données comportementales, démographiques et environnementales relatives aux chats domestiques (âge, sexe, stérilisation, habitudes de sortie, localisation, etc.).

Vous trouverez ci-joint un document officiel intitulé "Autorisation de collecte, traitement et diffusion des données relatives à un animal de compagnie", à remplir et signer si vous acceptez que les données concernant votre animal soient utilisées dans le cadre de cette recherche. Le document sera à renvoyer par mail au collecteur de données.
📎 Document joint : Felinomicon_fiche_autorisation.pdf

Nous vous assurons que toutes les données seront traitées de manière anonyme, dans le respect du RGPD, et exclusivement à des fins scientifiques et académiques.

👉 Important : vous pouvez à tout moment retirer votre consentement, sans justification, sur simple demande.

Nous restons à votre disposition pour toute question ou précision complémentaire.

En vous remerciant chaleureusement pour votre participation à ce projet de recherche,


Bien cordialement,
L'équipe de développement de Félinomicon.


Collecteur de données :
RODRIGUÈS Camille-Astrid / AMOUROUX Jan
💼 Développeur bio-informatique junior
📞 ********** | ✉️ ****************@*****.***
💻 https://github.com/CamilleAstrid/fr.univ-tlse3.Datamining_Ecological_impacts_of_pet_cats
```

Fiche d'autorisation ci-contre : [Felinomicon_fiche_autorisation](https://github.com/CamilleAstrid/fr.univ-tlse3.Datamining_Ecological_impacts_of_pet_cats/blob/main/data/French_dataset/Felinomicon_fiche_autorisation.pdf)

[Un exemplaire sera conservé par l'équipe de recherche](https://github.com/CamilleAstrid/fr.univ-tlse3.Datamining_Autorisation) et un autre exemplaire par les propriétaires.
